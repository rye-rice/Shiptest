...
/obj/structure/telephone_transmitter
	...
	icon = 'icons/obj/telephone/phone_obj.dmi'
...
/obj/structure/telephone_transmitter/Initialize(mapload, ...)
	...
	attached_receiver = new phone_type(src)
	RegisterSignal(attached_receiver, COMSIG_PREQDELETED, .proc/override_delete)
	update_icon()
...
/obj/structure/telephone_transmitter/proc/call_phone(var/mob/living/carbon/human/user, calling_phone_id)
	...
	if(!length(transmitters) || !(calling_phone_id in transmitters))
		...
		playsound(src, 'sound/telephone/call_fail.ogg', 20, FALSE, 14)
		return
	...
	playsound(src, 'sound/telephone/rtb_handset_1.ogg')
	...
	user.put_in_active_hand(attached_receiver)
	attached_receiver.setup_signal(user)

/obj/structure/telephone_transmitter/attack_hand(mob/user)
	. = ..()

	if(!attached_receiver || attached_receiver.loc != src)
		return
	...
	if(T.attached_receiver && ismob(T.attached_receiver.loc))
		var/mob/M = T.attached_receiver.loc
...
/obj/structure/telephone_transmitter/proc/reset_call(var/timeout = FALSE)
	...
	if(T)
		if(T.attached_receiver && ismob(T.attached_receiver.loc))
			...
			playsound(T.attached_receiver, 'sound/telephone/busy.ogg', 20, FALSE, 14)

		if(attached_receiver && ismob(attached_receiver.loc))
			var/mob/M = attached_receiver.loc
			if(timeout)
	...
	if(phone_caller)
		...
		if(attached_receiver.loc == src)
			if(next_ring < world.time)
				...
				playsound(phone_caller.attached_receiver, 'sound/telephone/ringback.ogg', 20, FALSE, 14)
		...
		var/obj/item/telephone_receiver/P = T.attached_receiver

		if(P && attached_receiver.loc == src && P.loc == T && next_ring < world.time)
			playsound(get_turf(attached_receiver), ringtone, 20, FALSE, 14)
			visible_message(span_warning("[src] rings vigorously!"))
			next_ring = world.time + 3 SECONDS
...
/obj/structure/telephone_transmitter/proc/recall_phone()
	if(ismob(attached_receiver.loc))
		var/mob/M = attached_receiver.loc
		M.dropItemToGround(attached_receiver)
		playsound(src, 'sound/telephone/rtb_handset_1.ogg', 100, FALSE, 7)

	attached_receiver.forceMove(src)
	reset_call()
...
/obj/structure/telephone_transmitter/Destroy()
	if(attached_receiver)
		if(attached_receiver.loc == src)
			UnregisterSignal(attached_receiver, COMSIG_PREQDELETED)
			qdel(attached_receiver)
		else
			attached_receiver.attached_receiver = null
			attached_receiver = null
...
/obj/item/telephone_receiver
	...
	var/obj/structure/telephone_transmitter/attached_receiver
...
/obj/item/telephone_receiver/proc/check_range()
	...
	if(!in_range(src,attached_receiver))
		var/mob/living/holder = loc
		if(istype(holder))
			to_chat(holder, "<span class='warning'>[src] overextends and come out of your hands!</span>")
		else
			visible_message("<span class='notice'>[src] flies back to [attached_receiver].</span>")
		forceMove(get_turf(attached_receiver))

/obj/item/telephone_receiver/proc/handle_speak(datum/source, list/speech_args, mob/living/speaker)
	SIGNAL_HANDLER

	if(!attached_receiver || loc == attached_receiver)
		UnregisterSignal(speaker, COMSIG_MOB_SAY)
		return

	attached_receiver.handle_speak(source, speech_args, speaker)
...
/obj/item/telephone_receiver/proc/handle_hear(datum/source, list/speech_args, mob/living/speaker)
	...
	var/obj/structure/telephone_transmitter/T = attached_receiver.get_calling_phone()
...
/obj/item/telephone_receiver/attack_hand(mob/user)
	if(attached_receiver && get_dist(user, attached_receiver) > attached_receiver.range)
		return FALSE
	return ..()
...
/obj/item/telephone_receiver/on_enter_storage(obj/item/storage/S)
	. = ..()
	if(attached_receiver)
		attached_receiver.recall_phone()
...
/obj/item/telephone_receiver/proc/do_zlevel_check()
	if(!attached_receiver || !loc.z || !attached_receiver.z)
		return FALSE

	if(zlevel_transfer)
		if(loc.z == attached_receiver.z)
			zlevel_transfer = FALSE
			if(zlevel_transfer_timer)
				deltimer(zlevel_transfer_timer)
			UnregisterSignal(attached_receiver, COMSIG_MOVABLE_MOVED)
			return FALSE
		return TRUE

	if(attached_receiver && loc.z != attached_receiver.z)
		zlevel_transfer = TRUE
		RegisterSignal(attached_receiver, COMSIG_MOVABLE_MOVED, .proc/transmitter_move_handler)
		return TRUE
	return FALSE
...
/obj/structure/telephone_transmitter/rotary
	...
	ringtone = 'sound/telephone/telephone_ring.ogg'
