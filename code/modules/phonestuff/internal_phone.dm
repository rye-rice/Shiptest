...
/obj/item/storage/backpack/radio
	...
	mob_overlay_icon = 'icons/obj/telephone/phone_mob.dmi'
...
/obj/item/storage/backpack/radio/update_icon()
	...
	if(!internal_transmitter.attached_receiver \
		|| internal_transmitter.attached_receiver.loc != internal_transmitter)
...
/obj/item/storage/backpack/radio/attack_hand(mob/user)
	...
	if(human_user.backpack == src)
		...
		if(internal_transmitter.attached_receiver && internal_transmitter.attached_receiver.loc != internal_transmitter)
			return . = ..()
		internal_transmitter.attack_hand(user)
	else
		. = ..()
...
/obj/item/storage/backpack/radio/attackby(obj/item/weapon, mob/user)
	if(internal_transmitter && internal_transmitter.attached_receiver == weapon)
